...
class RefreshStoreFromConfig:
    ...
    def _load_settings(self):
        ...
        video_target_dir = self.config.get_video_target_dir()
        ...
        if not self.store.get_state(BATCH_JOB_RUNNING): # only if project loaded by user, otherwise it alters start encoding frame in batch mode
            current_frame = 0
            current_frame = self.config.get_current_frame()
            settings[CURRENT_FRAME] = current_frame

        encode_all_frames = self.config.get_encode_all_frames()
        settings[ENCODE_ALL_FRAMES] = encode_all_frames

        frame_from = self.config.get_frame_from()
        settings[FRAME_FROM] = frame_from

        frame_to = self.config.get_frame_to()
        ...
        film_type = self.config.get_film_type()
        settings[FILM_TYPE] = film_type

        rotation_angle = self.config.get_rotation_angle()
        settings[ROTATION_ANGLE] = rotation_angle

        stabilization_threshold = self.config.get_stabilization_threshold()
        settings[STABILIZATION_THRESHOLD] = stabilization_threshold

        low_contrast_custom_template = self.config.get_low_contrast_custom_template()
        settings[LOW_CONTRAST_CUSTOM_TEMPLATE] = low_contrast_custom_template
        

        extended_stabilization = self.config.get_extended_stabilization()
        settings[EXTENDED_STABILIZATION] = extended_stabilization

        custom_template_defined = self.config.get_custom_template_defined()
        settings[CUSTOM_TEMPLATE_DEFINED] = custom_template_defined

        if custom_template_defined:
            template_name = self.config.get_custom_template_name()
            settings[CUSTOM_TEMPLATE_NAME] = template_name

            custom_template_expected_pos = self.config.get_custom_template_expected_pos()
            settings[CUSTOM_TEMPLATE_EXPECTED_POS] = custom_template_expected_pos

            full_path_template_filename = self.config.get_custom_template_filename()
        ...
        perform_cropping = self.config.get_perform_cropping()
        settings[PERFORM_CROPPING] = perform_cropping

        perform_denoise = self.config.get_perform_denoise()
        settings[PERFORM_DENOISE] = perform_denoise

        perform_sharpness = self.config.get_perform_sharpness()
        settings[PERFORM_SHARPNESS] = perform_sharpness

        perform_gamma_correction = self.config.get_perform_gamma_correction()
        settings[PERFORM_GAMMA_CORRECTION] = perform_gamma_correction

        gamma_correction_value = self.config.get_gamma_correction_value()
        settings[GAMMA_CORRECTION_VALUE] = gamma_correction_value

        crop_rectangle = self.config.get_crop_rectangle()
        settings[CROP_RECTANGLE] = crop_rectangle   # Top-left = crop_rectangle[0], bottom-right = crop_rectangle[1]

        force_4_3 = self.config.get_force_4_3()
        settings[FORCE_4_3] = force_4_3

        force_16_9 = False if force_4_3 else self.config.get_force_16_9()
        settings[FORCE_16_9] = force_16_9

        frame_fill_type = self.config.get_frame_fill_type()
        settings[FRAME_FILL_TYPE] = frame_fill_type

        generate_video = self.config.get_generate_video()
        settings[GENERATE_VIDEO] = generate_video

        video_filename = self.config.get_video_filename()
        settings[VIDEO_FILENAME] = video_filename

        video_title = self.config.get_video_title()
        settings[VIDEO_TITLE] = video_title

        # Snake case from the start
        skip_frame_regeneration = self.config.get_skip_frame_regeneration()
        settings[SKIP_FRAME_REGENERATION] = skip_frame_regeneration

        ffmpeg_preset = self.config.get_ffmpeg_preset()
        settings[FFMPEG_PRESET] = ffmpeg_preset

        perform_stabilization = self.config.get_perform_stabilization()
        settings[PERFORM_STABILIZATION] = perform_stabilization

        stabilization_shift_y = self.config.get_stabilization_shift_y()
        settings[STABILIZATION_SHIFT_Y] = stabilization_shift_y

        stabilization_shift_x = self.config.get_stabilization_shift_x()
        settings[STABILIZATION_SHIFT_X] = stabilization_shift_x

        perform_rotation = self.config.get_perform_rotation()
        settings[PERFORM_ROTATION] = perform_rotation

        video_fps = self.config.get_video_fps()
        settings[VIDEO_FPS] = video_fps

        video_resolution = self.config.get_video_resolution()
        settings[VIDEO_RESOLUTION] = video_resolution

        current_bad_frame_index = self.config.get_current_bad_frame_index()
        settings[CURRENT_BAD_FRAME_INDEX] = current_bad_frame_index

        user_defined_left_stripe_width_proportion = self.config.get_user_defined_left_stripe_width_proportion()
